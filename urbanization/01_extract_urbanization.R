...
######################################################
# 1. Create new rasters of urban change 1996-2016
revalue_ras <- function(file_1, file_2){
  
  # Read in file1
  ras_1 <- file_1 %>% raster() %>% crop(extent(bolivar)) %>% raster::mask(bolivar)
  
  # Extract all values labelled as urban, based on legend from ESA CCI
  ras_1 <- ras_1 == 190
  
  # Read in file2
  ras_2 <- file_2 %>% raster() %>% crop(extent(bolivar)) %>% raster::mask(bolivar)
  
  # Extract all values labeled as urban, based on legend from ESA CCI
  ras_2 <- ras_2 == 190
...
######################################################
# 2. Extract area where urban cover has decreased

files     <- list.files(path = "output", pattern = ".tif", full.names = TRUE)
ras_stack <- stack(files)

# Extract cells where urban has increased (1)
ras_stack <- ras_stack == 1

# Function to calculate annual total area where urban cover has decreased, for each polygon
...
calc_area <- function(parroquia, municipio){
  ...
  for (i in 1:nlayers(ras_polygon)) {
    ...
    if (all(is.na(c(values(ras)))) == TRUE){
      a_df <- data.frame(urban_increase = 0,
                         Parroquia       = parroquia,
                         Municipio       = municipio,
      ...
      a_df <- as.data.frame(zonal(a, ras, "sum")) %>%
        subset(zone == 1) %>%
        dplyr::rename(urban_increase = sum) %>% 
...
######################################################
# 3. Calculate cumulative urban increase and save data

data %>% dplyr::select(Year, Parroquia, Municipio, urban_increase_annual) %>% 
dplyr::group_by(Parroquia, Municipio) %>%
mutate(urban_increase_cum = cumsum(urban_increase)) %>% write.csv("data_output/data.csv")
